...
// available.
// Returns the latestTick when bucket is empty.
func (tb *Bucket) Wait(count int64) int64 {
	sleepDuration := tb.Take(count)
	if sleepDuration > 0 {
		tb.clock.Sleep(sleepDuration)
		return tb.latestTick
	}
	return 0
}
...
//
func (tb *Bucket) ModifyRateBySpeed(speed float32) {
	tb.mu.Lock()
	defer tb.mu.Unlock()
	var deltaQuantum = int64(float32(time.Second.Nanoseconds()/tb.fillInterval.Nanoseconds()) * speed)
	tb.quantum += deltaQuantum
}
